...
Production: see [docs/DEPLOY.md](docs/DEPLOY.md) (Render free web service via `render.yaml`; Vercel also works). The radar refresh and agent tick run every 10 minutes: a pinger calls `POST /api/agent`, which does the work in the background and writes to Upstash; [`.github/workflows/sonar-tick.yml`](.github/workflows/sonar-tick.yml) is the fallback (GitHub throttles its cron to every few hours).
